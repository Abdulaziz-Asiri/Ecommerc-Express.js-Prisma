...
const authMiddelware = async(req: Request, res:Response, next:NextFunction)=>{
    ...
    //2. if thoen isnot present, throw an error of unauthorized
    if(!token){
    ...
    try{
        ...
        if (!user){
            next(
            new UnauthorizedException("Unauthorizd", ErrorCode.UNAUTHORIZED)
            );
        }
        ...
        next(new UnauthorizedException('Unauthorizd', ErrorCode.UNAUTHORIZED))
    }
}
